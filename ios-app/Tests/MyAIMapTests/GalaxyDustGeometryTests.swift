...
struct GalaxyDustGeometryTests {
    ...
    @Test func dustSitsNearerThanStars() {
        // Dust is foreground haze: its shell must be inside the star shell so
        // it reads between the nodes and the far stars, not as more stars.
        #expect(GalaxyDustGeometry.shellRadius < StarFieldGeometry.shellRadius)
    }
