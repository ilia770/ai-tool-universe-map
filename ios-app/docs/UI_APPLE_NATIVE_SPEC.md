...
> **Reference, not binding — 2026-08-09.** The normative framing this document
> originally carried was not adopted: the canonical role and process contract
> is the one matrix that already exists, and a second binding layer beside it is
> what left three disagreeing descriptions of who does what in this project.
> Read this as a description of how the UI is built, not as a gate that has to
> be satisfied before touching it. The rejected proposal, and the reasoning, are
> on the `rejected/agent-ui-process` branch.
