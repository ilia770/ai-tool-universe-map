...
| --- | --- | --- | --- |
| Stop-ship | Catalog transfer interactive/device evidence | The v2 document, reversible migration, backup/recovery, native export/import, reset confirmation, and recovery UI now have 433/433 full-unit and 1/1 recovery-UI simulator evidence. The current Settings export/import entry smoke still needs a healthy-simulator runtime rerun; native Files/exporter cancellation and device behavior remain unobserved. | Pass the current `CatalogRecoveryUITests` on a healthy simulator, manually exercise Files/exporter cancellation and backup restore, then record the device matrix. |
